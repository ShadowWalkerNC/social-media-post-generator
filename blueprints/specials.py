...
Specials schedule CRUD endpoints + page route.

Routes:
  GET  /schedule                     -- schedule management page
  GET  /api/specials                 -- list all specials for current user
  POST /api/specials                 -- create a new special
  PUT  /api/specials/<id>            -- update an existing special
  DELETE /api/specials/<id>          -- delete a special
  POST /api/specials/<id>/cancel     -- cancel a queued/pending special

A 'special' is one row in the specials table: a named item the user wants
the agent to post on a specific date and time, across chosen platforms.
The AutomationAgent reads pending specials every hour and generates + schedules them.
"""
...
# Platforms the UI should offer checkboxes for (LinkedIn/Pinterest excluded)
UI_PLATFORMS = [
...
def api_update_special(special_id):
    ...
    # Only allow editing pending specials (queued/published cannot be undone)
    try:
        from app import get_db
        db  = get_db()
        row = db.execute(
            'SELECT status FROM specials WHERE id = ? AND user_id = ?',
            (special_id, uid)
        ).fetchone()
    except Exception:
    ...
    if row['status'] not in ('pending',):
        return jsonify({
            'success': False,
            'error':   f'Cannot edit a special with status "{row["status"]}". Cancel it first.'
        }), 409
    ...
    if 'post_date' in data:
        try:
            datetime.strptime(data['post_date'], '%Y-%m-%d')
            fields.append('post_date = ?'); values.append(data['post_date'])
        except ValueError:
            return jsonify({'success': False, 'error': 'post_date must be YYYY-MM-DD'}), 400
    if 'post_time' in data:
        try:
            datetime.strptime(data['post_time'], '%H:%M')
            fields.append('post_time = ?'); values.append(data['post_time'])
        except ValueError:
            return jsonify({'success': False, 'error': 'post_time must be HH:MM'}), 400
    ...
    try:
        db.execute(
            f'UPDATE specials SET {", ".join(fields)} WHERE id = ? AND user_id = ?',
            values
        )
        db.commit()
...
# ---------------------------------------------------------------------------
# API: Cancel (sets status=cancelled without deleting)
# ---------------------------------------------------------------------------
