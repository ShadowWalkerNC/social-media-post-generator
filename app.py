#!/usr/bin/env python3
"""
Social Media Post Generator - Main Flask App
...
from modules.generator import SocialMediaPostGenerator
from modules.meta_api import MetaAPI
from modules.scheduler import PostScheduler
...
def api_analytics():
    data = request.json
    analytics = Analytics(data.get('access_token'), data.get('page_id'))
...
def auth_facebook():
    ...
    url = f"https://www.facebook.com/v19.0/dialog/oauth?client_id={app_id}&redirect_uri={redirect_uri}&scope={scopes}"
    return redirect(url)
...
def auth_facebook_callback():
    ...
    # Get Instagram ID linked to page
    ig_res = req.get(
