...
async function generateWeekly() {
    ...
    renderSchedule(data.schedule, 'weeklyResult');
}
...
async function generateSingle() {
    ...
    renderSchedule([data.post], 'singleResult');
}

// ─── Render Posts ────────────────────────────────────────────
function renderSchedule(posts, targetId) {
    let html = '';
    posts.forEach((post, i) => {
        const isFb = post.platform === 'facebook';
        const badge = isFb ? '<span class="badge badge-fb">📘 Facebook</span>' : '<span class="badge badge-ig">📸 Instagram</span>';
        html += `
            <div class="post-card ${isFb ? 'fb' : ''}">
                <div class="post-header">${badge}<span class="post-time">🕐 ${post.post_time}</span></div>
                <pre class="post-caption">${escHtml(post.caption)}</pre>
...
function copyCaption(i) {
    navigator.clipboard.writeText(window._posts[i].caption).then(() => alert('Caption copied!'));
}

// ─── Publish Now ─────────────────────────────────────────────
async function publishPost(platform, caption) {
    const imageUrl = prompt('📸 Enter image URL (or leave empty for text-only):');
...
function showScheduler(caption, platform) {
    const dt = prompt('📅 Enter date & time to schedule (e.g. 2026-06-23T08:00:00):');
    if (!dt) return;
    const imageUrl = prompt('📸 Enter image URL (or leave empty):');
...
async function bulkSchedule() {
    ...
    alert('📅 Bulk scheduling coming in Phase 3! Connect Facebook to schedule from the calendar.');
}
